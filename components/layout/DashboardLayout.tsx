...
export function DashboardLayout({
  ...
  return (
    <AnalysisProgressProvider>
      ...
      `}</style>
      <div className="app-shell" data-space={currentRoute?.space ?? 'folio'} data-focus-mode={focusedChatRoute || undefined}>
        <button
          type="button"
      ...
      >
        ...
        <main id="main-content" tabIndex={-1} className={cn("shell-main custom-scrollbar focus:outline-none", focusedChatRoute && "overflow-hidden flex flex-col")}>
          <div aria-hidden="true" className="shell-route-wash" />
          <div className={cn('relative min-h-full', focusedChatRoute ? 'flex-1 flex flex-col pb-0' : 'pb-[calc(5.75rem+env(safe-area-inset-bottom))] lg:pb-0')}>{children}</div>
        </main>
