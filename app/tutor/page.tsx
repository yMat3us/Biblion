...
export default function TutorPage() {
  ...
  const handleScroll = () => {
    ...
    userScrolledUpRef.current = scrollHeight - clientHeight > scrollTop + 15
  }

  useEffect(() => {
    if (scrollContainerRef.current) {
      if (!userScrolledUpRef.current || messages.at(-1)?.role === 'user') {
  ...
  const addFiles = (files: FileList | null) => {
    ...
    if (hasOversized) {
      toast.error('Algumas imagens ultrapassam o limite de 20MB e foram ignoradas.')
    }
  ...
  return (
    <div data-archetype="cabinet" className="chat-shell tutor-cabinet flex-1 min-h-0 relative">
      <header className="tutor-header pl-2">
        <Link 
          href="/dashboard"
          className="flex h-9 w-9 items-center justify-center rounded-lg text-subtle transition-colors hover:bg-elevated hover:text-foreground"
          aria-label="Voltar para o Dashboard"
        >
          <ChevronLeft size={20} />
        </Link>
        <span className="tutor-header__seal ml-1 hidden sm:flex">
