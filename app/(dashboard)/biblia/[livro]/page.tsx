                              ...
                              <div className="my-4 rounded-2xl border border-primary/20 bg-elevated/90 p-4 shadow-inner sm:ml-8 sm:p-6">
                                {!insight ? (
                                  <div className="flex flex-col items-center py-5 text-center">
                                    ...
                                    <h3 className="mt-4 text-lg font-semibold text-foreground">Revelação e Profundidade</h3>
                                    <p className="mt-2 max-w-lg text-sm leading-6 text-muted-foreground">Deixe o assistente extrair a exegese, a aplicação prática e preparar o caminho para o seu sermão baseado nesta passagem.</p>
                                    <Button type="button" loading={aiLoading === verse.verse} onClick={() => fetchInsights(verse.verse, verse.text)} className="mt-5">
                                      <Sparkles size={16} /> Receber inspirações
                                    </Button>
                                  </div>
                                ) : (
                                  <div>
                                    ...
                                    {insight.comparacaoVersoes?.length > 0 && (
                                      <div className="mt-4 rounded-2xl border border-hairline bg-background/30 p-5">
                                        ...
                                        <div className="mt-4 grid gap-3 sm:grid-cols-2">{insight.comparacaoVersoes.map((comparison) => <div key={`${comparison.versao}-${comparison.texto}`} className="rounded-xl border border-hairline bg-surface p-4"><Badge variant="outline">{comparison.versao}</Badge><p className="mt-2 font-serif text-sm italic leading-6 text-muted-foreground">“{comparison.texto.trim().replace(/^["'“”«»]+|["'“”«»]+$/g, '')}”</p></div>)}</div>
                                      </div>
                                    )}
                                  </div>
                                )}
