...
async function buscarAnotacoes(cursor?: string): Promise<{ anotacoes: Anotacao[]; nextCursor: string | null }> {
  const data = await apiFetch<{ items: AnotacaoApiResponse[]; nextCursor: string | null }>(
    cursor ? `/api/anotacoes?cursor=${encodeURIComponent(cursor)}` : '/api/anotacoes',
  )
  return { anotacoes: data.items.map(normalizeNote), nextCursor: data.nextCursor }
}
...
export default function AnotacoesPage() {
  ...
  const refreshNotes = async () => {
    const { anotacoes: data, nextCursor } = await buscarAnotacoes()
    setAnotacoes(data)
  ...
  const carregarMais = async () => {
    ...
    setLoadingMore(true)
    try {
      const { anotacoes: mais, nextCursor } = await buscarAnotacoes(cursor)
      setAnotacoes((current) => [...current, ...mais])
      setCursor(nextCursor)
    } catch {
  ...
  useEffect(() => {
    ...
    void buscarAnotacoes()
      .then(({ anotacoes: data, nextCursor }) => {
        if (!cancelled) {
          setAnotacoes(data)
          setCursor(nextCursor)
          setLoadStatus('success')
