...
const STAT_META = [
  { key: 'sermoes', label: 'Sermões', descriptor: 'mensagens no altar', icon: Mic, href: '/sermoes', tone: 'primary', index: 'I' },
  { key: 'esbocos', label: 'Esboços', descriptor: 'sementes de pregação', icon: FileText, href: '/esbocos', tone: 'info', index: 'II' },
  { key: 'anotacoes', label: 'Anotações', descriptor: 'revelações diárias', icon: BookMarked, href: '/anotacoes', tone: 'scripture', index: 'III' },
  { key: 'licoes', label: 'Lições EBD', descriptor: 'ensinos e discipulado', icon: GraduationCap, href: '/ebd', tone: 'success', index: 'IV' },
] as const

const QUICK_PATHS = [
  { href: '/busca', label: 'Investigar um tema', description: 'Cruze passagens, sermões e ricas lições.', icon: Search },
  { href: '/teologia', label: 'Doutrina e Teologia', description: 'Navegue pelas profundezas da fé cristã.', icon: LibraryBig },
  { href: '/ebd', label: 'Preparar o ensino', description: 'Organize suas revistas e lições da EBD.', icon: GraduationCap },
] as const
...
export function DashboardClient({
  ...
  userName: string
  today: string
  dailyVerse: { text: string; reference: string; book: string; chapter: string }
}) {
    ...
    >
      <motion.section variants={item} className="dashboard-folio folio-surface" data-folio="FÓLIO 01">
        <div className="dashboard-folio__intro">
          ...
          <h1>
            A graça e a paz, <span>{userName}</span>.<br />
            Este é o seu momento devocional.
          </h1>
          <p>Aprofunde-se na Palavra de Deus, guarde Suas promessas no coração e prepare-se para o serviço do Reino.</p>
          <div className="mt-7 flex flex-col gap-3 sm:flex-row">
            <Link href="/biblia" className={buttonStyles({ size: 'lg', className: 'sm:min-w-44' })}>
              <BookOpen size={18} /> Mergulhar nas Escrituras <ArrowRight size={15} />
            </Link>
            <Link href="/sermoes/novo" className={buttonStyles({ variant: 'outline', size: 'lg' })}>
              <Plus size={17} /> Preparar nova mensagem
            </Link>
          </div>
        </div>

        <div className="dashboard-folio__scripture">
          <div className="dashboard-folio__chapter" aria-hidden="true">{dailyVerse.chapter}</div>
          <p className="index-label text-scripture">O Pão Nosso de Cada Dia</p>
          ...
          <div className="mt-5 flex items-center justify-between gap-4 border-t border-scripture/15 pt-4">
            ...
            <Link href={`/biblia/${encodeURIComponent(dailyVerse.book)}`} className="inline-flex items-center gap-1.5 text-xs font-semibold text-scripture transition-colors hover:text-foreground">
              Meditando no contexto <ArrowRight size={13} />
            </Link>
          </div>
        </div>
      </motion.section>
      ...
      <motion.section variants={item} className="dashboard-ledger" aria-labelledby="archive-summary-title">
        <div className="dashboard-ledger__heading">
          <div>
            <p className="index-label">Seu Ministério</p>
            <h2 id="archive-summary-title">Frutos da sua dedicação</h2>
          </div>
      ...
      <div className="mt-5 grid gap-5 xl:grid-cols-12">
        <motion.section variants={item} className="manuscript-surface xl:col-span-8" aria-labelledby="recent-activity-title">
          <div className="dashboard-section-heading">
            <div>
              <p className="index-label">Jornada Espiritual</p>
              <h2 id="recent-activity-title">Últimas reflexões</h2>
            </div>
          ...
          {feed.length > 0 ? (
            ...
            <div className="flex min-h-72 flex-col items-center justify-center px-6 py-12 text-center">
              ...
              <h3 className="font-serif text-xl font-semibold text-foreground">Sua jornada começa aqui.</h3>
              <p className="mt-2 max-w-sm text-sm leading-relaxed text-muted-foreground">Registre sua primeira mensagem, esboço ou anotação de estudo bíblico.</p>
            </div>
          )}
        </motion.section>
        ...
        <motion.aside variants={item} className="space-y-5 xl:col-span-4">
          <section className="cabinet-surface p-5 sm:p-6" aria-labelledby="tutor-card-title">
            <div className="mb-7 flex items-center justify-between gap-3">
              ...
              <span className="index-label text-primary-hover">Assistente Teológico</span>
            </div>
            <h2 id="tutor-card-title" className="max-w-xs font-serif text-[1.65rem] font-semibold leading-tight text-foreground">Descubra os tesouros da Palavra.</h2>
            <p className="mt-3 text-sm leading-7 text-muted-foreground">Converse sobre contexto, receba conexões profundas e conselhos para preparar seus estudos.</p>
            <Link href="/tutor" className={buttonStyles({ size: 'md', className: 'mt-6 w-full' })}>
              <Sparkles size={16} /> Consultar o assistente <ArrowRight size={14} />
            </Link>
          </section>

          <section className="marginalia-surface p-5 sm:p-6" aria-labelledby="continuity-title">
            <p className="index-label text-scripture">Caminhe Mais Longe</p>
            <h2 id="continuity-title" className="mt-2 font-serif text-xl font-semibold text-foreground">Deixe o Espírito guiar seu estudo.</h2>
