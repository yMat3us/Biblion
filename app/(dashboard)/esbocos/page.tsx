...
async function buscarEsbocos(cursor?: string): Promise<{ esbocos: EsbocoLocal[]; nextCursor: string | null }> {
  const data = await apiFetch<{ items: EsbocoApiResponse[]; nextCursor: string | null }>(
    cursor ? `/api/esbocos?cursor=${encodeURIComponent(cursor)}` : '/api/esbocos',
  )
  return { esbocos: data.items.map(normalizeOutline), nextCursor: data.nextCursor }
}
...
export default function EsbocosPage() {
  ...
  const carregarMais = async () => {
    ...
    setLoadingMore(true)
    try {
      const { esbocos: mais, nextCursor } = await buscarEsbocos(cursor)
      setEsbocos((current) => [...current, ...mais])
      setCursor(nextCursor)
    } catch {
      toast.error('Não foi possível carregar mais esboços.')
    } finally {
