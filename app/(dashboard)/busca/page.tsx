...
const DISCOVERY_CARDS = [
  { icon: BookOpen, title: 'Busque pelo coração', description: 'Pesquise "fé para superar o luto" sem precisar saber o capítulo exato.' },
  { icon: BrainCircuit, title: 'Profundidade Teológica', description: 'Nossa inteligência relaciona a sua busca com doutrinas, contextos e a vida cristã.' },
  { icon: LibraryBig, title: 'Seu ministério em foco', description: 'Seus sermões, lições e esboços passados ressurgem para iluminar o presente.' },
]
...
export default function BuscaInteligentePage() {
  ...
  return (
    <WorkspacePage size="wide" archetype="atlas">
      <PageHeader
        variant="atlas"
        index="Buscador Teológico · IA"
        eyebrow={<><Search size={13} /> Conexões Espirituais</>}
        title={<>Procure uma direção. <span className="text-gradient">Encontre a revelação.</span></>}
        description="Atravesse as Escrituras, compreenda doutrinas e conecte a Palavra aos seus próprios sermões e anotações em um único lugar."
        aside={
        ...
        {status === 'idle' && (
          ...
          >
            <div className="atlas-discovery__legend">
              <p className="index-label text-info">Como a Inteligência trabalha</p>
              <h2 id="atlas-discovery-title">A união entre a Palavra, a Doutrina e o seu Chamado.</h2>
              <p>O Biblion não busca apenas palavras, ele entende o peso teológico da sua busca e traz respostas ancoradas na Verdade.</p>
              <div aria-hidden="true" className="atlas-discovery__coordinates">
                <span>A BÍBLIA</span><span>A TEOLOGIA</span><span>O SEU MINISTÉRIO</span>
              </div>
            </div>
        ...
        {status === 'success' && results && hasAnyResult && (
          ...
          >
            <section aria-labelledby="ai-insights-title">
              <SectionHeading
                icon={Sparkles}
                title={<span id="ai-insights-title">Inspiração Guiada</span>}
                description={`Passagens e conexões para consolar e ensinar sobre “${searchedQuery}”.`}
                  ...
                  {results.aiResults.versiculos.map((verse) => (
                    <article key={`${verse.referencia}-${verse.texto}`} className="reader-paper p-5 sm:p-6">
                      <div className="relative">
                        ...
                        <blockquote className="mt-4 font-serif text-lg leading-8 text-foreground/90 sm:text-xl">“{verse.texto.trim().replace(/^["'“”«»]+|["'“”«»]+$/g, '')}”</blockquote>
                ...
                <div className="space-y-5">
                  ...
                  {results.aiResults.sugestaoSermao && (
                    <article className="surface form-section--accent p-5 sm:p-6">
                      ...
                      <p className="mt-4 font-serif text-base italic leading-8 text-foreground/90">“{results.aiResults.sugestaoSermao.trim().replace(/^["'“”«»]+|["'“”«»]+$/g, '')}”</p>
