...
const POLL_MS = 4000
// Aba oculta: espaça o polling (backoff) para poupar rede, bateria e cota de
// rate limit. Ao voltar a ficar visível, sincroniza imediatamente.
const POLL_HIDDEN_MS = 60000
// Com o SSE conectado, o polling vira apenas rede de segurança (cadência lenta).
const POLL_SSE_MS = 20000
...
export function ThreadClient({
  ...
  const sincronizar = useCallback(async () => {
    try {
      ...
      // silencioso: a próxima iteração (polling/SSE) tenta de novo
    }
  }, [conversaId, adicionar, marcarLida])

  // Tempo real: mensagens desta conversa disparam sincronização imediata; o
  // estado da conexão SSE ajusta a cadência do polling de fallback.
  const conectadoRef = useRef(false)
  useLiveEvents(
    useCallback(
      (event: LiveEvent) => {
        if (event.type === 'message.sent' && event.conversationId === conversaId) void sincronizar()
      },
      [conversaId, sincronizar],
    ),
    useCallback((connected: boolean) => {
      conectadoRef.current = connected
    }, []),
  )
  ...
  useEffect(() => {
    ...
    const proximoIntervalo = () => {
      ...
      // Com SSE conectado, o polling é só rede de segurança (cadência lenta).
      return conectadoRef.current ? POLL_SSE_MS : POLL_MS
    }

    const agendar = () => {
      timer = setTimeout(executar, proximoIntervalo())
    }
    ...
    const aoMudarVisibilidade = () => {
      ...
      // Voltou a ficar visível: sincroniza já e retoma a cadência.
