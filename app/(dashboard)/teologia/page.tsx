      ...
      <AnimatePresence mode="wait" initial={false}>
        {!doutrinaAtual ? (
          ...
          >
            <PageHeader
              variant="atlas"
              index="Índice Sistemático · Doutrinas"
              eyebrow={<><BookMarked size={13} /> Alicerces da Fé</>}
              title={<>Verdades eternas que sustentam a <span className="text-gradient">Igreja.</span></>}
              description={`Mergulhe nas ${DOUTRINAS.length} grandes doutrinas cristãs. Compreenda os fundamentos da nossa fé com sólida base bíblica e aplicação guiada pelo Espírito.`}
              aside={
          ...
          >
            <DetailHeader
              variant="atlas"
              index={`Fundamento ${String(DOUTRINAS.findIndex((entry) => entry.id === doutrinaAtual.id) + 1).padStart(2, '0')} · Índice Sistemático`}
                        ...
                        >
                          <div className="border-t border-hairline p-4 sm:p-6">
                            <div className="prose max-w-none text-sm sm:text-base">
                              <Markdown>{topico.conteudo}</Markdown>
                            </div>
