...
interface RouteOptions {
  ...
  /**
   * Nomes de params de rota que são ObjectId (24 hex). Validados ANTES do handler
   * (e do Prisma); formato inválido responde 404 — anti-enumeração e consistente
   * com "recurso inexistente", sem depender de erro do banco.
   */
  objectIdParams?: readonly string[]
}

async function assertObjectIdParams(ctx: unknown, names: readonly string[]): Promise<void> {
  const params = await (ctx as { params?: Promise<Record<string, string>> } | undefined)?.params
  for (const name of names) {
    if (!isObjectId(params?.[name])) throw ApiErrors.notFound()
  }
}
...
export function route<Ctx = unknown>(
  ...
  const { auth = true, rateLimit: limitOptions, objectIdParams } = options
  ...
  return async (req: NextRequest, ctx: Ctx): Promise<Response> => {
    try {
      ...
      // Valida os ObjectId de rota após autenticar (401 antes de 404) e antes do handler.
      if (objectIdParams?.length) await assertObjectIdParams(ctx, objectIdParams)
