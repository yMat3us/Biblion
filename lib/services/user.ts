...
import { ingestDataUrlToStorage } from '@/lib/storage'
...
async function removeObsoleteSessions(userId: string): Promise<void> {
  try {
    ...
    logger.error('session_physical_cleanup_failed', { error })
  }
}
...
export const UserService = {
  ...
  create: async (actor: AuthUser, input: AccountCreateInput) => {
    ...
    await assertStrongPassword(input.password, { username: input.username })
  ...
  updateProfile: async (userId: string, input: ProfileUpdateInput) => {
    ...
    // Se o avatar veio como imagem embutida e há object storage configurado,
    // sobe para o bucket e guarda só a URL pública. Sem storage (ou em falha),
    // mantém o data URL — o banco continua funcionando (fallback).
    if (avatarUrl?.startsWith('data:')) {
      const hosted = await ingestDataUrlToStorage(avatarUrl, userId)
  ...
  updateAccount: async (actor: AuthUser, targetId: string, input: AccountUpdateInput) => {
    ...
    if (input.newPassword) await assertStrongPassword(input.newPassword)
  ...
  changePassword: async (userId: string, currentPassword: string, newPassword: string) => {
    ...
    await assertStrongPassword(newPassword, { username: user.username })
