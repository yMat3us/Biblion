...
// Teto de solicitações de amizade pendentes (enviadas e ainda não respondidas).
// Mesmo sob rate limit, sem este teto um usuário poderia acumular milhares de
// solicitações ao longo do tempo, usando a notificação como vetor de spam.
export const MAX_PENDING_SENT_REQUESTS = 50
...
export const SocialService = {
  ...
  sendRequest: async (me: string, alvoId: string) => {
    ...
    if (existing) {
      ...
      await Events.emit({ type: 'friend.accepted', recipientId: alvoId, actorId: me })
    ...
    if (pendingSent >= MAX_PENDING_SENT_REQUESTS) {
      throw ApiErrors.tooManyRequests(
        'Você atingiu o limite de solicitações de amizade pendentes. Aguarde respostas antes de enviar novas.',
      )
    }

    await prisma.friendship.create({ data: { requesterId: me, addresseeId: alvoId, status: FriendshipStatus.PENDING } })
    await Events.emit({ type: 'friend.requested', recipientId: alvoId, actorId: me })
  ...
  respond: async (me: string, solicitanteId: string, aceitar: boolean) => {
    ...
    if (aceitar) {
      ...
      await Events.emit({ type: 'friend.accepted', recipientId: solicitanteId, actorId: me })
      return { status: 'friends' as RelationshipStatus }
    }
