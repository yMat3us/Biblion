...
describe('ChatService — controle de acesso', () => {
  ...
  it('envia a mensagem e notifica o destinatário entre amigos', async () => {
    ...
    // A notificação foi desacoplada do request: o envio enfileira um evento no
    // outbox; a criação da notificação ocorre fora do caminho crítico (after()/cron).
    expect(prisma.outboxEvent.create).toHaveBeenCalledWith(
      expect.objectContaining({ data: expect.objectContaining({ type: 'message.sent' }) }),
    )
  })
...
describe('NotificationService — coalescência de mensagens', () => {
  ...
  it('atualiza a notificação existente em vez de criar outra', async () => {
    ...
    await NotificationService.createMessage('bob', 'alice', 'c1')
