...
describe('account RBAC and session revocation', () => {
  ...
  it('não força perfil público nem descoberta ao criar conta (usa o default privado do schema)', async () => {
    await UserService.create(actor(UserRole.OWNER), {
      username: 'nova.conta',
      password: 'Senha segura 123!',
      role: UserRole.MEMBER,
    })

    const data = mocks.prisma.user.create.mock.calls[0]?.[0]?.data as Record<string, unknown>
    // Contas novas devem herdar os defaults privados do schema (FRIENDS/false).
    // A criação não pode sobrescrevê-los para PUBLIC/true.
    expect(data.profileVisibility).toBeUndefined()
    expect(data.isSearchable).toBeUndefined()
  })
