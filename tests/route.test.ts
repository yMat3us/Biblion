...
describe('route security boundary', () => {
  ...
  it('rejects a malformed ObjectId route param with 404 before running the handler', async () => {
    const observed = vi.fn()
    const handler = route<{ params: Promise<{ id: string }> }>(
      async () => {
        observed()
        return Response.json({ ok: true })
      },
    ...
    const response = await handler(request('GET'), { params: Promise.resolve({ id: 'not-an-object-id' }) })
  ...
  it('allows a well-formed 24-hex ObjectId route param through to the handler', async () => {
    const handler = route<{ params: Promise<{ id: string }> }>(
      async () => Response.json({ ok: true }),
      { objectIdParams: ['id'] },
    )

    const response = await handler(request('GET'), { params: Promise.resolve({ id: '507f1f77bcf86cd799439011' }) })
